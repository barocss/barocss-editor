...
#248 기준점 필수 CI 확인 중에는 임시 공간에서 초안을 준비했다. 의존성 재검토 결과 편집기 코드와 독립적이므로 코드 PR은 현재 최신 main에서 별도로 만든다. 기준점 병합 후 새 main을 반영하고 전체 필수 검사를 다시 확인한다. #251 정책은 사용자가 확인한 Changesets 제품 버전 합의를 따른다. 이 작업은 #249의 model/datastore 제품 구현을 변경하지 않는다. 최종 CI에서 발견한 기존 순회 성능 테스트 불안정성은 #257로 추적하고 테스트만 보강한다.
