...
- [ ] **뷰 층이 두 벌 — 1단계 끝, 2단계에서 진짜 논리 차이가 나왔다.**
      **1단계(선택 어휘를 `shared` 로) 끝.** 근거가 바뀌어 되살아난 단계다: 원래 근거인 *"그래프가
      순환처럼 읽힌다"* 는 사라졌고(DAG), 지금 근거는 **두 뷰 층의 DOM↔모델 변환을 그 둘 아래에
      두려면 타입도 아래여야 한다** 이다. `editor-core` 가 다시 내보내므로 118개 파일이 안 바뀌었다.

      **2단계(열한 메서드)를 재보니 모든 의존이 `getNode` 하나다** — `this.editor` 를 쓰는 것은 둘뿐,
      나머지 아홉은 순수 DOM. 사이의 둘을 합쳐 열셋, **326줄**(파일 771줄 중).

      **그런데 기계적이지 않다.** 여섯을 대보니 여섯 다 몸통이 다르고, `isDecoratorElement` 는 표기가
      아니라 **논리**가 다르다: DOM 판은 `data-decorator-category !== 'inline'` 을 묻고 React 판은 안
      묻는다. **DOM 판이 맞다** — 인라인 데코레이터는 런의 일부인 글자를 감싸므로 그것을 빼면 런
      색인에서 글자가 빠지고 오프셋이 밀린다. 즉 **React 경로에 잠재 결함이 하나 더 있다**(아직 검사로
      재지 않았다). 여섯 쌍마다 이런 판단이 필요하고, 그 판단을 프로세에 적는 것이 이 작업의 값이다.
