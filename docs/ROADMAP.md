...
- [ ] **드래그 열여섯 개를 `dragGesture` 로.** 넷은 옮겼다(`office-ui/stack`, `slide/layer-panel`,
      `slide/paint-panel`, `word/ruler`). 남은 것은 `pointercancel` 을 안 들어서 리스너를 `window` 에
      영구히 남긴다 — `TECHNICAL-ROADMAP.md` §2.0 에 위험한 순서로.
