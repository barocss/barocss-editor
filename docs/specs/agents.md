...
아래는 **같은 작업 디렉터리를 공유하는 대화형 세션**의 규칙이다. 향후 Wonffice 무인 실행기는 [별도 실행 계약](wonffice-agent-runtime.md)에 따라 관리 clone과 작업별 worktree를 사용한다. 기준 commit을 명시하고 선행 변경을 확인한다. 이 설계는 현재 작업 폴더의 변경을 옮기거나 초기화할 권한이 아니다.
