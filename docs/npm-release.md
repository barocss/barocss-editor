...
New packages need an initial registration before their npm settings can be configured. Use the explicitly selected `bootstrap` authentication mode for that first batch and set `NPM_BOOTSTRAP_TOKEN` in the `npm` environment. The token must have publish access to the intended `@barocss` packages and satisfy the organization's 2FA policy. After registration, configure each trusted publisher, remove the bootstrap secret and use `trusted` mode. A scoped package requires permission in the npm organization; owning the GitHub organization alone is not enough.
