...
| 브라우저 검사 | **스위트 여섯** — note 20 · word 375 · slide 407 · site 283 · **editor-react 14** · **editor-test** | `ls apps/*/playwright.config.ts` |
...
**스위트가 여섯인데 한동안 넷만 돌고 있었다.** `editor-react`(7 스펙·14 검사)와 `editor-test` 가
빠져 있었고, 그 사이에 `editor-view-react` 를 고쳤다 — 확인 없이 고친 것이다. 루트에
`test:e2e:react` 스크립트가 있는데도 그랬다. **규칙이 아니라 목록이 틀렸고, 목록은 세어야 한다.**
