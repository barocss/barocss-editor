...
### 방향을 두 번 뒤집었다 — 그리는 쪽 넷이 이미 합의한 것을 안 읽고 (2026-09-06)

페이지 설정 대화상자에서 「가로」를 골랐는데 종이가 안 눕었다. 문서에는 정확히 쓰여 있었다:
`pageWidth: 15840, pageHeight: 12240, orientation: 'landscape'`. 그런데 그려진 것은 612pt,
그대로 세로.

원인은 내가 규약을 반대로 읽은 것이다. `layout.ts` 에서 여백 계산을 105줄부터 읽었고 **96줄의
한 줄을 놓쳤다**:
...
그러고는 *"`layout.ts` 는 `orientation` 을 보지 않는다 — 폭과 높이만 본다"* 를 모델의 머리말에
적고, 그 위에 *방향은 두 변의 관계* 라는 설계를 세웠다. 그래서 저장값을 맞바꾸고 이름도 적었고,
그리는 쪽이 한 번 더 뒤집어 제자리로 돌아왔다.

**규약은 넷이 합의하고 있었다** — `layout.ts:96`, `css.ts:381`(인쇄용 페이지 상자),
`css.ts:421`(`pageCss`), `canvas-insert.ts:141`. 넷 다 같은 두 줄이다: *저장값은 세운 상태의 두
변, `orientation` 이 눕히라는 지시.*

세 가지가 값을 했다:

| | |
|---|---|
| `changes()` | *"종이의 모양이 바뀌지 않았습니다 — 0.77 그대로입니다"* 로 즉시 잡았다 |
| 짐작 대신 탐침 | 문서를 찍어 보니 **문서는 옳고 그림이 틀렸다** — 범위가 절반으로 줄었다 |
| `grep orientation` | 넷이 나왔고 내 문장이 틀렸다는 것이 한 줄로 드러났다 |

그리고 A4 검사는 그 사이에도 **초록이었다.** 폭은 낡은 채로 높이만 바뀌어 비율이 달라졌기
때문이다 — 공짜 초록이 진짜 결함을 가린 첫 사례다.

**모았다** — `@barocss/shared` 의 `sideways(landscape, {width, height})`. 넷이 그것을 부르고,
대화상자가 다섯 번째 독자다. 속성 이름은 모른다(`pageWidth` 도 `orientation` 도 없다) — 그것들은
오피스의 낱말이고 `shared` 는 낱말을 모르는 자리다. 여기 있는 것은 두 수와 한 판단뿐이다.

`shared/src/units/sideways.test.ts` 가 **여섯 번째 사본**을 찾는다. 그 검사도 두 번 틀렸고 둘 다
적어 둘 값어치가 있다:

| | |
|---|---|
| 주석을 코드로 읽었다 | *이 규약이 무엇이었는지* 설명하는 문단 때문에 빨개졌다. **문서를 지우게 만드는 검사는 잘못된 검사다** — 주석을 걷고 본다 |
| 아무것도 못 잡았다 | 삼항의 갈래를 `[\w.?]*` 로만 봐서 `landscape ? number(…)` 의 괄호를 못 넘었다. 되돌려 심어 보고서야 알았다 |

**잡는지 확인하지 않은 검사는 없는 것과 같다.** 이 저장소가 이미 여러 번 겪은 것이고(내 단위
필터, 브라우저 분류기, 백로그 검사, 데코레이터 정규식, 산문 검사), 이번에는 심어서 확인했다.


### 페이지를 여는 문단은 자기 앞 간격을 잃는다 (2026-09-06)

문단 간격 대화상자의 브라우저 검사를 쓰다 나왔다. 새 문서의 첫 문단이 `margin-top: 96px` 을
갖고 있었다 — 서식이 아니라 **페이지네이션**이다:

```ts
// office-text/src/renderers/block-style.ts:169
const push = getBlockPush(env, String(node.sid ?? ''));
if (push !== undefined) style.marginTop = `${push}px`;   // 덮어쓴다
```

`getBlockPush` 의 주석이 그것을 말한다 — *"the block that opens a page is pushed down to meet its
sheet."* 그래서 **페이지를 여는 문단에서는 `spacingBefore` 가 화면에 나타나지 않는다.**

Word 도 페이지 맨 위에서는 앞 간격을 죽이므로 **결과는 맞다.** 문제는 그것이 *결정*으로 적혀
있지 않다는 것이다. 지금은 `=` 한 글자의 부수 효과이고, 다음 사람이 `+=` 로 고치면 Word 와
달라지면서도 아무 검사도 빨개지지 않는다.

**적었고 세웠다** — `block-style.ts` 의 그 줄에 이유가 붙었고, `office-text/test/
block-that-opens-a-page.test.ts` 가 셋을 묻는다: 페이지 안에서는 앞 간격을 그리는가, 페이지를
열면 밀어 내린 값이 이기는가, **앞 간격이 0 이어도 그런가**(규칙이 값에 달려 있지 않다는 것).
`+=` 로 바뀌면 둘째가 빨개진다.

### 대화상자 하나가 검사 셋을 헛돌게 했다 — 세 번 (2026-09-06)

「문단 간격」의 브라우저 검사가 세 번 헛돌았고, 세 번 다 **바꾸려는 값과 지금 값이 같아서**였다:

| 헛돈 것 | 왜 |
|---|---|
| 첫 문단의 `margin-top` | 페이지네이션이 96px 로 덮고 있었다 (위 항목) |
| `1.5줄` 을 골랐을 때 줄 높이 | 이미 24px, 즉 글꼴 크기의 1.5배였다 |
| `selectOption('1.5')` | `ChoiceSelect` 는 Radix 이지 네이티브가 아니다 |

셋째는 오류로 드러나므로 값이 싸다. 앞의 둘은 **초록으로 지나간다.** 테두리 때 배운 것과 같은
모양이고(픽스처가 이미 입고 있었다 · preflight 가 `solid` 를 깔았다), 이제 다섯 번째다.

- [ ] *바꾸기 전과 후가 다른가* 를 묻는 것을 브라우저 검사의 기본 모양으로 삼을 것. 지금은 검사마다
      손으로 쓰고, 안 쓰면 아무도 모른다.


### 하네스가 이름만 보고 있었다 — 96개가 한 번에 조용해질 뻔했다 (2026-09-06)

Word 에 첫 대화상자를 만들었다. 「테두리 및 음영」 — `word.md` 가 갚아야 할 것 다섯 묶음 중 첫째로
적어 둔 열여섯 개다. 스키마도 그리는 쪽도 처음부터 있었고 **쓰는 쪽만 없었다.**

다 만들고 재니 `every-property-can-be-edited` 가 **184 → 88**. 96개다. 열여섯을 갚았는데 아흔여섯이
사라진다면 그건 갚은 것이 아니다. 검사를 읽었다:

```ts
if (settable.has(attr)) continue;   // 이름만. 노드가 없다.
```

대화상자는 **문단에만** 쓴다. 같은 `borderTopStyle` 이 표에도 셀에도 페이지에도 선언돼 있어서
셋이 함께 조용해졌고, 그중에는 **정말로 설정할 곳이 없는 셀 테두리**가 있었다.

고친 뒤: 검사가 `node.attr` 도 받는다(맨이름은 여전히 *어디서든*, 글꼴 크기가 그렇다). Word 는
`borderEditable()` 로 자기가 정말 쓰는 짝만 내놓는다 — 노드는 명령에서, 속성은 `borderPatch`
**에게 물어서**. **184 → 136**, 48개. 문단·제목·목록 항목 셋. 표와 셀과 페이지는 그대로 빚이다.

이 저장소가 이 모양을 만난 것이 이번이 두 번째다. `every-attribute-is-read` 는 스키마를 걸으므로
*그려지는데 선언되지 않은* 속성은 주제조차 되지 않는다. **한쪽에서 열거하는 검사는 그쪽 방향의
구멍만 찾는다.**

**나머지 세 제품도 좁혀야 하나 — 재보니 아니었다.** 이 항목을 쓰자마자 확인했다. 사이트의
`editable` 은 `sitePanelAttrs()` 에서 오고, 그 패널의 절들은 `when: { attr, is }` 로 걸린다 —
**노드 종류가 아니라 속성값으로.** 즉 사이트의 패널은 정말로 *고른 것이 무엇이든* 그 속성을
답하고, 맨이름이 그쪽에서는 정확한 진술이다. 덱의 `slidesPanelAttrs()` 도 같은 모양이다.

그래서 같은 관대함이 한 제품에서는 옳고 다른 제품에서는 틀렸으며, 차이는 **어떤 종류의 표면이
답하느냐**다:

| 표면 | 무엇을 답하나 | 맞는 형식 |
|---|---|---|
| 패널 · 리본 | 고른 것이 무엇이든 | `attr` |
| 대화상자 | 자기가 여는 것 하나 | `node.attr` |

- [ ] 이 구분을 `every-property-can-be-edited` 의 머리말에 적을 것 — 지금은 두 형식을 받는다고만
      적혀 있고, **어느 쪽을 언제 쓰는지**가 없다. 다음 사람이 대화상자를 만들며 맨이름을 내놓으면
      96개가 다시 조용해진다.

### 픽스처가 입고 있어서 초록이었고, 리셋 CSS 라서 또 초록이었다 (2026-09-06)

테두리 대화상자의 브라우저 검사 다섯이 처음부터 초록이었다. 믿지 않고 *먼저 없다*를 확인하는 줄을
넣자 셋이 빨개졌다 — **샘플의 첫 문단은 이미 테두리를 입고 있었다.** 단추가 아무것도 안 해도 나오는
초록이었다.

새 문서(`createStarterDocument`)로 옮겼는데 **여전히** 빨갰다. Tailwind preflight 가
`*{ border: 0 solid }` 를 깐다: 이 문서의 모든 요소에서 `border-*-style` 은 언제나 `solid` 이고
두께가 `0px` 다. 즉 단언이 문단이 아니라 **리셋 CSS** 를 확인하고 있었다.

`border-*-width` 로 바꾸고 시작점을 `0px` 로 못 박았다.

- [ ] `toHaveCSS(..., 'solid')` 를 쓰는 다른 검사가 있는지 쓸어볼 것. 이 저장소의 모든 앱이 같은
      preflight 를 쓰므로, 같은 방식으로 공짜 초록인 단언이 더 있을 수 있다.
...
- [x] **캐럿이 블록 경계에 있으면 선택으로 읽혔다.**

  `findBestContainer` 는 경계가 블록 요소일 때 시작을 *그 안의 첫 런*, 끝을 *마지막 런* 으로
  내려보낸다(`forEnd`). 두 경계가 **같은 자리** 였어도 — 캐럿이었어도 — 서로 다른 런으로 갈라진다.
  재본 것: 런 둘을 가진 문단의 요소 경계에 캐럿을 두면 `t1:0 → t2:0`, **`collapsed=false`**.

  `range.collapsed` 를 아무도 안 묻고 있었다. **브라우저가 이미 답한 것을 해석이 잃었다.**

  화면에 나타난 자리: 사이트에서 `/` 를 치면 슬래시 메뉴와 **버블 툴바가 같이** 떴다(떠 있는 표면이
  둘이라 검사가 *strict mode violation* 으로 실패했다). 버블 툴바는 `collapsed !== true` 면 뜨고,
  그 이유가 그 파일에 적혀 있다 — *"캐럿은 선택이 아니다: `applyMark` 는 0글자에 아무것도 안 한다."*

  두 뷰 층 다 고쳤고 검사를 양쪽에 뒀다(`boundary-inside-a-block.test.ts`).

  ## 그리고 그 실패의 원인을 내가 잘못 지목했다

  *"React 에 걷기를 더해서 site 가 깨졌다"* 고 적었다. **`apps/site` 는 `editor-view-react` 를 아예
  쓰지 않는다** — `package.json` 에도 소스에도 없다. 닿을 수 없는 인과였다.

  같은 회차에 `editor-view-dom` 에도 바뀐 것이 있었다: `findFirstTextNode` 가 자기 판정 대신
  `shared` 의 `skipsInIndex` 를 쓰게 됐고, 그 둘의 답이 한 경우에 다르다 —
  `data-bc-decorator="inline"` 인데 `data-decorator-category` 속성이 없는 요소. 옛 판은 *자기 것을
  그리는 것*(건너뛴다), 새 판은 *인라인*(걷어 들어간다).

  **회차 셋으로 갈랐다:**

  | 회차 | 결과 | 실패한 것 |
  |---|---|---|
  | 캐럿 수정 **전** | 1 실패 / 282 | `site.spec.ts:8298` — `/` 메뉴와 버블 툴바가 같이 뜸 |
  | 캐럿 수정 **전** (다시) | 1 실패 / 282 | `site.spec.ts:1062` — 클릭 선택 타임아웃 |
  | 캐럿 수정 **후** | **283 / 283** | — |

  **두 회차가 서로 다른 검사에서 실패한 것이 오히려 근거다.** 캐럿이 선택으로 읽히면 한 자리가 아니라
  *선택을 읽는 모든 자리* 가 흔들린다 — 떠 있는 표면이 둘이 되고, 클릭이 기다리던 것을 못 찾는다.

  그래도 **내가 처음 지목한 인과는 틀렸다**: *"React 에 걷기를 더해서"* 라고 적었는데 `apps/site` 는
  `editor-view-react` 를 아예 쓰지 않는다. 맞은 것은 *캐럿이 갈라진다* 이고, 그건 `editor-view-dom`
...
- **제품 키맵이 `editorEditable` 을 하나도 안 건다.** 🔴 열림
  ...
  재본 것: Word **54개 중 0개**, note **2개 중 0개**. 엔진은 40 중 23이다.

  **일괄로 거는 것이 답이 아니다** — `copy` 와 `selectAll` 은 읽기 전용에서 **되어야** 한다. 어느
  명령이 읽기 전용에서 살아야 하는가는 제품의 결정이고, 그 결정은 읽기 전용을 처음 낼 때 내린다.
  그때까지 이 항목이 그 목록을 기다린다.

- **제품의 키가 어디서 도는지가 둘로 갈려 있다.** 🔴 열림 — 결정이 필요하다

  | 제품 | 방법 |
  |---|---|
  | word · note | 엔진 레지스트리(`Keybinding[]` + `when`) |
  | slides · site | **데이터 + 호스트 디스패처** — `SLIDES_KEYS`/`SITE_KEYS`(둘 다 `office-controls` 의 `KeyModel`)를 앱이 걸어 돈다 |

  **둘 다 논증이 적혀 있다.** `slides/keymap.ts` 의 머리: *"what a product binds is a fact about the
  product, and how the press is caught is the host's. The overlay is what catches them, because only
  it knows whether the reader is typing in a box or holding one."*

  재보니 그 논증의 **절반은 이제 안 맞는다**: `needsSelection` 은 엔진이 이미 공짜로 답한다
  (`selectionEmpty`·`selectionType` 이 내장 맥락이다). 남는 절반은 맞다 — *크롬의 입력칸에
  타이핑 중인가* 는 엔진이 모른다.

  그리고 slides/site 는 **맥락을 하나도 안 세운다**(`setContext` 호출 0). `when` 을 쓰는 것은
  word 뿐이다.
...
- **`editorFocus` 는 *문서를 만지고 있다* 를 뜻하지 않는다.** 🔴 열림
  ...
  `focus`/`blur` 를 **콘텐츠 층**에 붙여서 세운다. 그러므로 문서 표면이 더 넓은 제품에서는 무대에
  초점이 있을 때 **거짓**이다. word 의 관례(`gates every binding on editor focus`)를 slides 가
  그대로 따르면 모든 바인딩이 죽는다.

  당장은 slides 가 `selectionType` 으로만 묻게 해서 피한다. 제대로 된 답은 `focusin`/`focusout` 을
  문서 표면에 붙이는 것이고, 그건 1,056개 브라우저 검사가 걸린 변경이라 따로 잰다.

- **slides 이주의 크기를 쟀다.** 🔴 다음

  `SLIDES_KEYS` 스물넷 = **문서 키 20 · 엔진 재진술 2**(⌘Z·⌘⇧Z) **· 크롬 2**(⌘S·F5).

  - `needsSelection` 은 **공짜다** — `selectionType == 'node'` 이고 문법은 이미 지원된다.
  - 진짜 맥락 일은 **하나**: 오버레이가 넘기는 `parentId: inside ?? slideSid`(*"오직 오버레이만
    아는 것"*). `Keybinding.args` 는 정적이라 못 싣는다. 그걸 **실제로 읽는 명령은 하나**
    (`pasteBoxes`)이므로 오버레이가 맥락으로 세우고 그 명령이 거기서 읽으면 된다.
  - 재진술 둘은 keySurface 이주 **뒤에** 걷어야 한다. 지금 걷으면 도형을 고른 채 누르는 ⌘Z 가
    죽는다 — 그 자리에서는 엔진이 못 듣기 때문이다. **순서가 있는 이주다.**

  **그리고 무대를 그대로 건넬 수는 없다.** 재보니 `<Stage>`(app.tsx:2016)와
  `<SelectionOverlay>`(2081)가 **형제**이고 둘의 공통 조상은 `.sl-main`(1881)인데, 그 안에
  **`NotesPane` 도 있다**(2112) — 발표자 노트는 *두 번째 `EditorViewDOM`* 이다. `.sl-main` 을
  표면으로 삼으면 노트에 타이핑하는 키가 덱의 문서 키로 읽힌다(노트의 선택은 `range` 지만 덱의
  선택은 여전히 `node` 다).

  그러므로 **무대와 오버레이만 감싸는 요소**가 필요하고, 그건 레이아웃 구조 변경이다 —
  407개 브라우저 검사가 걸린다. 이주의 첫 조각이 그것이고, 따로 잰다.

- **`apps/note` 의 인용문 검사는 내 회귀가 아니라 제품의 간헐 결함이다.** 🔴 열림

  `note.spec.ts:213` 이 전체 회차에서 이따금 실패한다. 내가 선택 계산을 건드린 뒤였으므로 단독
  재실행을 증거로 쓰지 않고 **커밋된 상태로 되돌려 3회** 를 따로 쟀다:
  ...
  실패의 내용이 세 번 다 같다: `atEndOf` 는 **통과** 하고(캐럿은 문단 끝에 제대로 있다),
  그 다음 `Enter` 가 **5초 동안 줄을 안 만든다** — `blockquote p` 가 1에서 안 늘어난다.

  그러므로 캐럿 해석이 아니라 **인용문 안에서의 Enter** 다. 5초는 키 하나에 긴 시간이므로
  검사의 성급함이 아니라 제품이 이따금 아무것도 안 하는 것이다. 다음에 팔 자리.
...
- [ ] **`NoteEditor` 가 호스트의 폭을 20px 줄인다.** 끌기 손잡이가 설 자리로
  ...
  사이트의 브라우저 검사 283개가 그대로 통과하므로 **잰 범위에서는 깨진 것이 없다.** 그래도 적어 두는
  이유는 그것이 이 패키지가 호스트의 레이아웃에 하는 변경이고, **감당 못 하는 호스트가 그렇게 말할
  방법이 오늘 없다**는 것이다. 손잡이를 글의 왼쪽 여백 위에 겹치게 하면 상자를 안 넓히지만, 왼쪽
  여백이 없는 블록에서는 첫 글자를 가린다. 둘 중 무엇이 나은지는 재 본 적이 없다.

- [ ] **`apps/site` 스위트가 회차 하나에서 6 실패 / 58분이 나왔다** (평소 283/283 · 12.6분).
  같은 트리로 단독 재실행하니 **283/283 · 12.6분**이다. 여섯의 내용도 부하의 모양이었다 —
  `Target page, context or browser has been closed`, 30초 클릭 타임아웃, `getComputedStyle` 인자가
  null, 그리고 폭 검사가 `> 60` 을 기대하고 `0` 을 받은 것(= 아무것도 안 그려짐).

  **그리고 나는 원인을 잘못 지목할 뻔했다.** 마침 그 회차에 `NoteEditor` 에 20px 여백을 더했고,
  실패 여섯 중 둘이 폭 검사이며, 사이트가 그 뷰를 마운트한다 — 인과가 그럴듯했다. 고치는 스크립트까지
  써 두고 **단독 재실행을 먼저 했다.** 그럴듯한 인과는 측정이 아니다.


- [ ] **`note.spec.ts:213`(인용문)이 전체 회차에서만 흔들린다.** 2026-09-05 에 두 번 갈랐다:
  ...
  `addChild` 를 고친 회차에 실패했으므로 **인과를 의심할 이유가 있었다** — 그 수정이 인용문의 캐럿도
  한 단계 깊게 옮긴다(`blockQuote > paragraph > inline-text`). 그래서 단독이 아니라 **전체 회차를 다시**
  돌려 가렸다: 통과했다. 단독 통과만으로는 못 갈랐을 자리다.

  슬라이드·사이트의 `waitForTimeout` 318개 항목과 같은 뿌리로 보인다. 적어 두는 이유는 절차다:
  **내가 방금 그 자리를 건드렸을 때, 단독 재실행은 증거가 아니다.** 같은 방식으로 다시 돌려야 한다.


- [x] **표를 넣으면 캐럿이 구조 노드에 앉는다 — 그리고 클릭해도 안 고쳐진다.**

  `addChild` 의 `selectionAfter` 가 이랬다:

  ```ts
  const firstTextNodeId = addedNode.content[0] ? addedNode.content[0] : null;
  ```

  **`content[0]` 을 한 칸만 보고 그것을 `firstTextNodeId` 라 불렀다.** 문단은 `content[0]` 이 곧 글자
  런이라 맞았고, **표는 `bTableHeader`** 다. 이름이 값과 달랐다.

  노트에서 잰 것:

  | | 모델 | DOM |
  |---|---|---|
  | 2×2 표를 넣은 직후 | `range` on `bTableHeader` | 첫 칸의 런 |
  | 첫 칸을 클릭한 뒤 | **그대로** | 그대로 |

  **클릭이 안 고치는 이유가 결정적이다:** DOM 선택이 이미 그 자리이므로 브라우저가
  `selectionchange` 를 **띄우지 않는다**(0회를 셌다). 그래서 모델은 태어날 때의 잘못된 값에 영구히
  머문다. 타이핑하면 입력 경로가 DOM 에서 다시 계산해 복구된다 — 그래서 *써 보면 되는데* 로 보인다.

  결과: 캐럿으로 셀을 찾는 모든 것이 `null` 을 받는다 — `nextCell` · `insertRowBelow` ·
  `mergeCells`. 표를 가진 **네 제품 전부**의 문제다.

  **그런데 노트의 툴바가 그것을 가려 왔다.** `note-view.tsx` 가 눌린 칸을 명시적으로 넘긴다:
  `const on = { nodeId: sid, cellId: cell }`. 그래서 *행 추가* 단추는 되고 키보드는 안 됐다.
  **단추가 있는 길과 키보드의 길이 다르면, 단추가 결함을 가린다.**

  고친 자리는 `addChild` 하나다 — 깊이 여덟로 묶은 내려가기. `TableExtension._placeCaretIn` 이 같은
  걷기를 이미 갖고 있었고, 둘이 같은 답을 내야 한다.

- [x] **표에서 Tab 이 아무 일도 안 했다 — 표를 가진 넷 중 Word 만 통했다.**

  `nextCell`·`previousCell` 은 공용 `TableExtension` 이 등록한다. **키를 묶는 곳이
  `word-keymap.ts` 뿐이었다.** 그리고 노트에는 키맵이 아예 없었다.

  `word-keymap.ts` 의 프로세가 원칙을 적어 뒀다 — *"키는 제품의 것이고 엔진의 것이 아니다:
  `Mod+Alt+1` 은 워드프로세서에서 제목 1이고 FigJam 보드에서는 아무것도 아니다."* 맞다. 그런데
  **표에서의 `Tab` 은 제품의 관례가 아니라 그 노드의 관례다** — Word·Google Docs·Notion·스프레드시트가
  전부 같고, 표를 가진 제품이 그것을 각자 정할 여지가 없다.

  그래서 갈랐다: `Tab`/`Shift+Tab` 은 `TABLE_CELL_KEYBINDINGS` 로 **확장 옆에 한 번** 선언하고
  (`extensions/table.ts`), Word 는 그것을 펼치고 자기 두 줄을 뺐고, 노트는 `note-keymap.ts` 로
  등록한다. `Mod+Alt+i`(아래에 행) 류는 Word 의 발명이므로 Word 에 남는다.

  얹는 것이지 대체가 아니다 — `word-kit.ts` 에 그 이유가 적혀 있다: 레지스트리가 출처로 충돌을 풀고
  **제품 바인딩이 엔진 것을 이긴다.** 그래서 표 밖에서는 기본의 `Tab → indentText` 가 그대로 산다.

  **노트의 키맵이 둘인 것이 이 제품이 무엇인지 말한다** — Word 는 71개다. 엔진 기본이 본문을 쓰는 데
