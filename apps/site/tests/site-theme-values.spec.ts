...
const OU = {
  light: {
    ...
    /** 다크 블록이 다시 말하지 않는다 — 그래서 네 상태 모두 흰색이다. 아래 §대비 참고. */
 ...
 * 백로그에 넘긴다:
 *
 * - `--ou-accent-ink`(#ffffff) on `--ou-accent` — 라이트 **5.17**, 다크 **3.68**. 다크 블록이
 *   악센트만 밝히고(`#2563eb`→`#3b82f6`) 그 위의 잉크는 흰색으로 뒀다. 눌린 툴바 단추와
 *   `.st-back:hover` 의 글자가 그 쌍이다.
 * - `--ou-board-ink` on `--ou-studio` — 라이트 **3.98**, 다크 5.29. 보드 위의 이름표.
 */
