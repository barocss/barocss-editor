...
async function mockLogin(page: Page, options: { role?: Role; tenants?: number; apiStatus?: number; accessStatus?: number } = {}) {
  const { role = 'editor', tenants = 1, apiStatus = 200, accessStatus = 200 } = options;
...
test('all direct product URLs fail closed before local document reads', async ({ page }) => {
  ...
  for (const product of ['note', 'word', 'slides', 'site']) {
    ...
    await expect(page.getByRole('heading', { name: '문서 접근 확인이 필요합니다' })).toBeVisible();
