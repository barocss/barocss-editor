...
Private Node.js/TypeScript service code, separate from Fastify. It stores tenants, workspaces, document **metadata**, OIDC identities and tenant memberships. It does not store document bodies. Fastify uses its membership query after verifying an OIDC access token; `/health/ready` remains 503 because the product storage path is incomplete.
...
| `wonffice_app` | Not an owner or member of the owner or backup role; no superuser/BYPASSRLS. Can read the current tenant and read/write its workspaces/document metadata. Can read only the verified identity's memberships. Cannot change memberships, provision tenants, read migration history, truncate, or change roles to the owner. |
...
Evidence covers concurrent/repeated migration, failed DDL rollback, history mismatch, real app-role privileges, no-context/cross-tenant RLS, composite references, connection re-use after failure, rejection of configured tenant defaults and backup membership, owner bootstrap/grant/revoke audit, and a fresh database restore with matching data/history hashes and restored access control. The separate local Keycloak browser check exercises two synthetic accounts against the real PostgreSQL-backed API. Capacity, external provider authorization, product saving, full backup scheduling and production installation remain unverified or unimplemented.
