...
The draft `0006_company_member_admin` migration adds a narrow owner-owned function boundary for existing company member administration. The app role receives EXECUTE on bounded list/change functions, not direct UPDATE on memberships or SELECT on the audit table. A verified caller's tenant and OIDC context are set for one transaction. The functions check the caller's current active owner/admin membership, restrict role changes and revocation to existing editor/viewer rows in that tenant, and write an actor/target/action/outcome/time/request-ID audit row in the same transaction. A denied request from a known identity is also audited. An unknown identity has no row to reference, so its denial is not recorded in this DB audit table; HTTP access still fails closed. Audit failure rolls back a change. The event table contains no OIDC subject, token or customer document body. The installed backup role can read it for a reviewed backup/restore procedure. Retention and verified human display identity remain undecided; a UUID alone must not authorize an ambiguous UI choice.

`pnpm --filter @barocss/office-service test:company:postgres` exercises two synthetic tenants, owner/admin versus editor/viewer, cross-tenant and owner protection, same-token revocation, audit failure, and a new-database restore of revoked membership and audit history. `pnpm --filter @barocss/office-api test:company:oidc` uses the protected local synthetic Keycloak fixture and two independent browser contexts with the real HTTP API and PostgreSQL restart. These local checks do not validate a production IdP, installation or Office management UI.
