...
- **wrapInBlockquote**: payload (optional). Behavior: wrap current block in blockQuote, or unwrap if already inside blockQuote. selectionAfter: caret stays in same text node. Exec tests: `packages/model/test/operations/wrapInBlockquote.exec.test.ts`.
