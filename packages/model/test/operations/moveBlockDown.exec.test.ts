...
describe('moveBlockDown operation (exec)', () => {
  ...
  it('블록을 아래로 이동해야 함', async () => {
    const op = globalOperationRegistry.get('moveBlockDown');
    expect(op).toBeDefined();

    const result = await op!.execute(
      { type: 'moveBlockDown', payload: { nodeId: 'para-2' } } as any,
      context
    );
  ...
  it('마지막 블록은 아래로 이동할 수 없음', async () => {
    const op = globalOperationRegistry.get('moveBlockDown');
    const result = await op!.execute(
      { type: 'moveBlockDown', payload: { nodeId: 'para-3' } } as any,
      context
    );

    expect(result.ok).toBe(false);
    expect(result.error).toContain('already at last position');
  ...
  it('inverse operation이 올바르게 생성되어야 함', async () => {
    ...
    expect(result.inverse.type).toBe('moveBlockUp');
