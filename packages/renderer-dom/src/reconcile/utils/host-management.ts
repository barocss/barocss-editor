...
export function createHostElement(
  ...
  if (childVNode.sid && !isAutoGeneratedSid(childVNode)) {
    dom.setAttribute(host, DOMAttribute.BC_SID, String(childVNode.sid));
  }
  // VNode top-level decorator identity: copy to DOM as data-decorator-* attributes
  const childWithDecorator = childVNode as VNode & { decoratorSid?: string; decoratorStype?: string };
  if (childWithDecorator.decoratorSid != null) {
    dom.setAttribute(host, DOMAttribute.DECORATOR_SID, String(childWithDecorator.decoratorSid));
  }
  if (childWithDecorator.decoratorStype != null) {
    dom.setAttribute(host, DOMAttribute.DECORATOR_STYPE, String(childWithDecorator.decoratorStype));
  }
  // Apply attrs (class, style, etc.; attrs may also contain data-decorator-* when set by VNodeBuilder)
  if (childVNode.attrs) {
    for (const [key, value] of Object.entries(childVNode.attrs)) {
      if (value !== undefined && value !== null) {
        dom.setAttribute(host, key, String(value));
      }
    }
  }
