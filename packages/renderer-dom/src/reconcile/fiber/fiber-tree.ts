        ...
        if (fiber.alternate?.child) {
          ...
          if (!prevChildAlternate) {
            ...
            if (alternateChild && childIndex === i) {
              // Exclude already matched alternate
              if (!matchedAlternateFibers.has(alternateChild)) {
                const alternateIdForIndexMatch = getVNodeId(alternateChild.vnode);
                const effectiveAlternateIdForIndexMatch =
                  alternateIdForIndexMatch && isAutoGeneratedSid(alternateChild.vnode)
                    ? undefined
                    : alternateIdForIndexMatch;
                const bothUnkeyed = !effectiveChildId && !effectiveAlternateIdForIndexMatch;
