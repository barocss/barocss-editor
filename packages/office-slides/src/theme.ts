...
/**
 * Every value in a set of attributes, with its slots filled in.
 *
 ...
 * every caller knowing which of its keys can hold a slot.
 */
export function resolveThemeAttrs<T extends Record<string, unknown>>(
  ...
  for (const [key, value] of Object.entries(attrs)) {
     ...
     * Into the lists, too.
     *
     * A shape's paints and effects are arrays of objects, and a slot inside one
    ...
    if (Array.isArray(value)) {
      const mapped = value.map((entry) => {
        ...
        for (const [innerKey, innerValue] of Object.entries(copy)) {
          if (Array.isArray(innerValue)) {
            ...
            const stops = innerValue.map((stop) => {
              ...
              for (const [stopKey, stopValue] of Object.entries(stopCopy)) {
                if (!isThemeRef(stopValue)) continue;
                changed = true;
                const resolved = resolveThemeValue(theme, stopValue);
          ...
          if (!isThemeRef(innerValue)) continue;
          changed = true;
          const resolved = resolveThemeValue(theme, innerValue);
    ...
    touched = true;
    const resolved = resolveThemeValue(theme, value);
