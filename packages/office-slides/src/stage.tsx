 ...
 * 심볼이 사는 모듈에서 곧장 가져온다.
 */
import { SLIDE_16_9, ZOOM_MAX, ZOOM_MIN, fitScale } from './geometry';
import { echoGap } from './timeline';
import { facingCss, pathCss, type Facing, type PathPoint } from './motion-path';
import { splitAdditive } from './motion-effects';
import { splitText, type TextUnit } from './text-units';
import type { TransitionFrom } from './motion';
...
/** What the stage is told — a name, because a package exports names rather than shapes. */
export interface StageProps {
  /** The element the **editor's view** mounts into: React makes it and stops. */
  host: React.RefObject<HTMLDivElement | null>;
  /**
   * The stage's own element (`.sl-stage`), handed **out**.
   *
   * The overlay, the show and the timeline all measure this box and listen on it. They used to
   * find it with `document.querySelector('.sl-stage')` from a sibling subtree; a ref says which
   * stage without guessing, which matters the moment a second one exists (the presenter window).
   */
...
export function Stage({
  ...
  /**
   * `.sl-stage` 자체. 밖에서 주면 그것을 쓰고, 아니면 제 것을 만든다.
   *
   * 형제 컴포넌트(선택 오버레이·발표·타임라인)가 이 요소를 재고 여기에 듣는다. 앱에 있을 때는
   * `document.querySelector('.sl-stage')` 로 집었는데, 그것은 **서브트리 밖 조회**다: 발표자 창처럼
   * 무대가 둘인 순간 어느 것을 집을지 말할 수 없고, 필름스트립의 축소판이 같은 sid 를 그리므로
   * 선택자 하나가 160px 짜리 사각형을 답으로 줄 수도 있었다. 그래서 **레퍼런스로 건넨다.**
   */
