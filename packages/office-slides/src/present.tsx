...
export interface PresentProps {
   ...
   * **어느 무대인가** — 클릭을 듣는 요소.
   *
   * 앱에 있을 때는 `document.querySelector('.sl-stage')` 였다. 그건 형제 컴포넌트의 서브트리를
   * 밖에서 집는 것이고, 두 가지가 틀린다: 무대가 둘일 때(발표자 창) 어느 것인지 말할 수 없고,
   * 이 조각이 패키지로 오면 *`.sl-stage` 라는 이름을 이 파일이 안다* 는 사실 자체가 결합이다.
   * `Stage` 가 `frame` 으로 자기 요소를 내주고, 앱이 그것을 여기로 건넨다.
   */
...
export function Present({
   ...
   */
  useEffect(() => {
    /* 무대는 **건네받는다** — 이 파일이 `.sl-stage` 를 찾아 나서지 않는다. */
  ...
  }, [host, go, triggers, onTrigger, jumps, onJump]);
