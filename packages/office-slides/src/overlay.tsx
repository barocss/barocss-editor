...
/*
 * 자기 배럴을 거치지 않는다 — 심볼이 사는 모듈에서 곧장. `./manipulate` 은 캔버스 층을
...
import { cropByHandle, type Crop } from './crop';
import {
  addPoint,
  movePoint,
  pathData,
...
import { namedBoxes } from './timeline';
import { paintsOf, type PaintStop } from './paints';
import { connectorRouteOf, connectorTrackOf } from './deck';
import {
  guideIsDropped,
  movedGuide,
  readGuides,
...
import { slideMenu } from './context-menu';
import {
  Menu,
  TextField,
  toDisplay,
...
/** What the overlay over a slide is told. */
export interface SelectionOverlayProps {
  ...
  /**
   * **어느 무대인가** — 이 층이 재는 상자, 그리고 sid 로 요소를 찾을 범위.
   *
   * 앱에 있을 때는 `document.querySelector('.sl-stage …')` 였다. 그 선택자가 *범위를 말한
   * 것* 자체는 옳았다 — 필름스트립의 축소판이 같은 슬라이드를 같은 sid 로 그리므로, 범위 없는
   * 조회는 오버레이에 160픽셀짜리 사각형을 답으로 준다(`thumbnail.tsx` 가 그 값을 적어 뒀다).
   *
   * 틀린 것은 **범위를 문자열로 말한 것**이다. 그러면 이 파일이 형제의 클래스 이름을 알아야
   * 하고, 무대가 둘일 때(발표자 창) 어느 것인지 말할 수 없다. `Stage` 가 `frame` 으로 자기
   * 요소를 내주고 앱이 여기로 건넨다 — 같은 범위, 이름 없이.
   */
...
export function SelectionOverlay({
  ...
  useLayoutEffect(() => {
    ...
    const find = () => {
      if (!slideSid || !stage) return setRect(null);
      /**
       * Inside the stage, and said so — **by the element, not by a class name.**
       *
       ...
       * handles in.
       *
       * That scoping used to be the string `.sl-stage`, which meant this file knew a
       * sibling's class and could not say *which* stage once there were two.
       */
       ...
       * that does not work.
       */
      setViewport(stage.getBoundingClientRect());
    };
    find();
  ...
  }, [host, slideSid, revision, tick, drawnAt]);
  ...
  const elementFor = useCallback(
    (sid: string) =>
      host?.current?.querySelector<HTMLElement>(`[data-bc-sid="${CSS.escape(sid)}"]`) ?? null,
    [host]
  );
