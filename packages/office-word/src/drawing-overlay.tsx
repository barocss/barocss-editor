 ...
 */
/**
 * sid 로 그려진 요소 — **건네받은 곳 안에서만** 찾는다.
 *
 * 이 파일은 `host` 를 이미 prop 으로 받으면서도 `document.querySelector` 를 여섯 번 썼다. 받은
 * 것을 안 쓰는 조회는 두 가지를 조용히 틀리게 한다: 한 페이지에 편집기가 둘이면 **다른 문서의**
 * 도형을 잡고, 인쇄용 사본(`w-print-copy`)처럼 같은 sid 가 여러 벌 있으면 **첫 번째**를 잡는다.
 * 둘 다 오버레이가 엉뚱한 사각형을 그리는 것으로만 보인다.
 */
const drawnAt = (host: HTMLElement | null, sid: string | null | undefined): HTMLElement | null =>
  sid ? (host?.querySelector(`[data-bc-sid="${sid}"]`) as HTMLElement | null) ?? null : null;

/**
 * 그림 위의 포인터에게 필요한 것 — 문서, 그리고 **그 문서가 그려진 곳**.
 *
 * `host` 는 측정의 기준이자 조회의 범위다. 오버레이는 화면 픽셀로 그려지고(그래서 확대 틀 **밖**에
 * 산다), 잡을 것은 그 안에 있다.
 */
export interface DrawingOverlayProps {
  editor: Editor | null;
  /** 편집기가 붙은 요소. 여기 없는 도형은 이 오버레이의 것이 아니다. */
  host: HTMLElement | null;
}

export function DrawingOverlay({ editor, host }: DrawingOverlayProps) {
