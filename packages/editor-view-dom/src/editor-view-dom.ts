...
export class EditorViewDOM implements IEditorViewDOM {
   ...
   *
   * A selection toolbar and a `/` menu are the two surfaces every editor of this kind has and this
   * one has neither. The reason is not that they are hard: the parts have been here all along and in
   * four layers, three of which were finished —
   *
   ...
   * reachable only from inside that system. So the fourth layer was the one nobody could reach, and
   * a surface that needs all four could not be built by a product at all — which is why the two that
   ...
   *
   * A floating surface is drawn by the **app**, in its own overlay, over boards it may have scaled
   * and panned — the site draws three of them at once. Coordinates relative to a content layer would
   * make every caller undo a transform this view knows nothing about. `getBoundingClientRect`'s
   * frame is the one both ends already share.
   *
   * ## A range, not a caret
   *
   * `getClientRects()` rather than `getBoundingClientRect()` on the range, because a selection that
   * wraps across lines has a bounding box covering the whole paragraph — and a toolbar centred on
   * *that* sits in the middle of the text rather than above the words. The **first** rectangle is the
   * line the selection starts on, which is where every tool of this kind puts it.
   */
  selectionRect(): DOMRect | null {
    return selectionRectIn(this.layers.content ?? null);
  }
