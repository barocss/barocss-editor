...
export const LogCategory = {
  RECONCILE: 'Reconcile',
  /** Anything on the path from a key or an event to the model. */
  EDITOR: 'Editor',
  INPUT: 'Input',
  FIBER: 'Fiber',
  VNODE: 'VNode',
...
const categoryFlags: Record<string, boolean> = {
  ...
  [LogCategory.EDITOR]: false,
  [LogCategory.INPUT]: false,
...
/**
 * Turn categories on from the browser, without a rebuild.
 *
 * A logger that can only be enabled by editing source and recompiling is one
 * nobody reaches for, which is how these ended up as unconditional
 * `console.log` in the first place — including on the path that runs tens of
 * times per keystroke.
 *
 *   localStorage.setItem('barocss:debug', 'Editor,TextInput')  // then reload
 *   localStorage.setItem('barocss:debug', '*')
 */
export const DEBUG_STORAGE_KEY = 'barocss:debug';

export function enableCategoriesFromStorage(): void {
  let raw: string | null = null;
  try {
    raw = typeof localStorage !== 'undefined' ? localStorage.getItem(DEBUG_STORAGE_KEY) : null;
  } catch {
    // Storage can throw on a blocked or partitioned origin; debugging is not
    // worth failing to load over.
    return;
  }
  if (!raw) return;
