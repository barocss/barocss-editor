...
import { element, data, slot, text, when, define, component, attr, portal, defineMark, defineDecorator, getGlobalRegistry } from '@barocss/dsl';
import { DataTemplate, ElementChild, ElementTemplate, PortalTemplate } from '../../src/types';
...
describe('DSL Functions Tests', () => {
  ...
  describe('define() function', () => {
    it('should create renderer definition with ElementTemplate', () => {
      ...
      if (definition.template.type === 'component' && typeof definition.template.component === 'function') {
        ...
        expect(elementTemplate.type).toBe('element');
        expect(elementTemplate.tag).toBe('div');
      }
    });

    it('should create renderer definition with function component', () => {
      const componentFn = (props: any, ctx: any) => element('div', { className: 'dynamic', id: props.id });
      const definition = define('dynamic-component', componentFn);
      
      expect(definition.type).toBe('renderer');
      expect(definition.nodeType).toBe('dynamic-component');
      expect(definition.template.type).toBe('component');
      if (definition.template.type === 'component' && typeof definition.template.component === 'function') {
        const elementTemplate = definition.template.component({ id: 'test-id' }, {} as any);
        expect(elementTemplate.attributes?.className).toBe('dynamic');
        expect(elementTemplate.attributes?.id).toBe('test-id');
      }
    });

    it('should allow redefining renderer (overwrite)', () => {
      const template1 = element('div', { className: 'first' });
      const template2 = element('div', { className: 'second' });
      
      define('redefinable', template1);
      const def1 = getGlobalRegistry().get('redefinable');
      if (def1?.template.type === 'component' && typeof def1.template.component === 'function') {
        const el1 = def1.template.component({}, {} as any);
        expect(el1.attributes?.className).toBe('first');
      }
      
      define('redefinable', template2);
      const def2 = getGlobalRegistry().get('redefinable');
      if (def2?.template.type === 'component' && typeof def2.template.component === 'function') {
        const el2 = def2.template.component({}, {} as any);
        expect(el2.attributes?.className).toBe('second');
      }
    });

    it('should allow element() to use defined renderer', () => {
      define('card', element('div', { className: 'card' }));
      const cardTemplate = element('card', { id: 'card1' });
      
      expect(cardTemplate.type).toBe('component');
      expect(cardTemplate.name).toBe('card');
      expect(cardTemplate.props).toEqual({ id: 'card1' });
    });

    it('should handle ExternalComponent definition', () => {
      const externalComponent = {
        mount: () => {},
        unmount: () => {}
      };
      const definition = define('external-comp', externalComponent);
      
      expect(definition.type).toBe('renderer');
      expect(definition.nodeType).toBe('external-comp');
      expect(definition.template.type).toBe('external');
      
      // ExternalComponent is registered in _renderers via get(), not via getComponent()
      // getComponent() is for components registered via registerComponent()
      const registry = getGlobalRegistry();
      const renderer = registry.get('external-comp');
      expect(renderer).toBeDefined();
      expect(renderer?.template.type).toBe('external');
      expect(typeof (renderer?.template as any).mount).toBe('function');
      expect(typeof (renderer?.template as any).unmount).toBe('function');
    });
  });
