...
import type { RendererDefinition, ExternalComponent, ContextualComponent, ExternalDescriptor } from '../src/index';
...
describe('defineMark + external()', () => {
  ...
  it('defineMark with element() stores in registry as before', () => {
    const reg = getGlobalRegistry();
    defineMark('bold', element('strong', { className: 'mark-bold' }, [data('text')]));
    const renderer = reg.getMarkRenderer?.('bold');
    expect(renderer).toBeDefined();
    expect((renderer as any)?.type).not.toBe('external');
  });

  it('defineMark with external(ReactComponent) stores ExternalDescriptor', () => {
    const reg = getGlobalRegistry();
    const BoldComp = (p: any) => p;
    defineMark('extBold', external(BoldComp));
    const renderer = reg.getMarkRenderer?.('extBold');
    expect(renderer).toBeDefined();
    expect((renderer as ExternalDescriptor)?.type).toBe('external');
    expect(typeof (renderer as ExternalDescriptor)?.reactComponent).toBe('function');
  });

  it('getMarkRenderer returns undefined for unregistered mark', () => {
    const reg = getGlobalRegistry();
    expect(reg.getMarkRenderer?.('nonexistent')).toBeUndefined();
  });

  it('external() returns ExternalDescriptor with reactComponent', () => {
    const fn = () => null;
    const desc = external(fn);
    expect(desc.type).toBe('external');
    expect(desc.reactComponent).toBe(fn);
  });

  it('external({ mount, unmount }) returns ExternalDescriptor with mount/unmount', () => {
    const mount = () => document.createElement('div');
    const unmount = () => {};
    const desc = external({ mount, unmount });
    expect(desc.type).toBe('external');
    expect(desc.mount).toBe(mount);
    expect(desc.unmount).toBe(unmount);
    expect(desc.reactComponent).toBeUndefined();
  });

  it('element() mark and external() mark can coexist for different types', () => {
    const reg = getGlobalRegistry();
    defineMark('elemMark', element('em', {}, [data('text')]));
    defineMark('reactMark', external(() => null));
    const elem = reg.getMarkRenderer?.('elemMark');
    const react = reg.getMarkRenderer?.('reactMark');
    expect(elem).toBeDefined();
    expect(react).toBeDefined();
    expect((react as ExternalDescriptor)?.type).toBe('external');
    expect((elem as any)?.type).not.toBe('external');
  });

  it('defineMark with external() does not add mark class attribute (unlike element() marks)', () => {
    const reg = getGlobalRegistry();
    defineMark('reactOnly', external(() => null));
    const renderer = reg.getMarkRenderer?.('reactOnly') as ExternalDescriptor;
    expect(renderer?.type).toBe('external');
    expect((renderer as any)?.attributes?.className).toBeUndefined();
  });
});
