...
| **Remote / Pattern / Generator (optional)** | editor-view-dom has RemoteDecoratorManager, PatternDecoratorConfigManager, DecoratorGeneratorManager. editor-view-react has only the basic DecoratorManager. Adding these would be needed for collaboration (remote decorators) or pattern-based decorators in the React view. |
