...
/**
 * **인라인 데코레이터가 낀 런의 오프셋** — `editor-view-dom` 의 같은 검사와 짝이다.
 *
 * 두 뷰 층이 같은 판단을 각자 덧걸렀고 **서로 다르게** 틀렸다. 그래서 검사도 두 곳에 있다: 한쪽만
 * 고치는 것이 이 저장소가 이미 두 번 한 실수이고(`isTextContainer`, 요소 경계 계산), 그 둘 다 이
 * 회차에 *한쪽에만 남아 있는 것* 으로 발견됐다.
 *
