...
describe('StrikeThroughExtension - toggleStrikeThrough', () => {
  beforeEach(() => {
  ...
  it('같은 텍스트 노드 내 range selection 에 대해 strikethrough toggleMark operation 을 생성한다', async () => {
    const dataStore = {
      getNode: (id: string) => {
        if (id === 'text-1') {
    ...
    const ext = new StrikeThroughExtension();
    ext.onCreate(editor);

    const cmd = (editor as any).__getCommand('toggleStrikeThrough');
